...
public abstract class Graph_Abstract implements I_Graph {
	...
	public I_Iterator DFSTraverse(Vertex v) {
		...
		I_Iterator it = getVertex();	//从图中未曾访问的其他顶点出发重新搜索
	...
	private void DFSRecursion(Vertex v, LinkedList list){
		v.setToVisited();
		list.insertLast(v);
		I_Iterator it = adjVertexs(v);//取得顶点v的所有邻接点
		for(it.first(); !it.isDone(); it.next()){
			Vertex u = (Vertex)it.currentItem();
	...
	public I_Iterator shortestPath(Vertex v) {
		...
		I_Iterator it = getVertex();//初始化，将v到各顶点的最短距离初始化为由v直接可达的距离
		for(it.first(); !it.isDone(); it.next()){
			Vertex u = (Vertex)it.currentItem();
		...
		for (int t=1;t<getVexNum();t++){//进行n-1次循环找到n-1条最短路径
			...
			I_Iterator adjIt = adjVertexs(k);	//取出k的所有邻接点
