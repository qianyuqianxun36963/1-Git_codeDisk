...
import 简.表.*;
import 难.树.BinaryTreeLinked;
import 难.树.SearchTable;
...
public class BSTree extends BinaryTreeLinked implements SearchTable{
	...
	private I_Node binTSearchRe(BinTreeNode rt, Object ele){
	...
	private I_Node binTSearch(BinTreeNode rt, Object ele){
		while(rt!=null){
			switch(strategy.compare(ele,rt.getData()))
			{
				case  0: return rt;					//等于
	...
	public I_Node min(BinTreeNode v){
		if (v!=null)
			while (v.hasLChild()) v = v.getLChild();
		return v;
	}
	public I_Node max(BinTreeNode v){
		if (v!=null)
